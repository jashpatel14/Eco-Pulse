...
const FEATURES = [
  ...
  { icon: <Layers size={26} />, color: '#f59e0b', bg: 'rgba(245,158,11,0.1)', title: 'BOM Version Control', desc: 'Maintain precise Bills of Materials. Automatically trace versions, components, operations, and suppliers across every revision.' },
  { icon: <ShieldCheck size={26} />, color: '#22c55e', bg: 'rgba(34,197,94,0.1)', title: 'Strict Governance', desc: 'Customizable multi-stage approval workflows, granular Role-Based Access Control, and immutable audit logs.' },
];
...
export default function LandingPage() {
  ...
  return (
    <div style={{ background: 'linear-gradient(160deg, #2d1515 0%, #1a0a0a 50%, #1e1010 100%)', minHeight: '100vh', color: '#e2e8f0', fontFamily: "'Inter', system-ui, sans-serif", position: 'relative', overflow: 'hidden' }}>
      
      {/* Background glows */}
      <div style={{ position: 'fixed', top: '-180px', left: '-180px', width: '550px', height: '550px', background: `radial-gradient(circle, ${brand}18 0%, transparent 65%)`, borderRadius: '50%', pointerEvents: 'none', zIndex: 0 }} />
      <div style={{ position: 'fixed', bottom: '-180px', right: '-180px', width: '600px', height: '600px', background: `radial-gradient(circle, ${brand}10 0%, transparent 65%)`, borderRadius: '50%', pointerEvents: 'none', zIndex: 0 }} />

      {/* ─── Nav ─── */}
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 6%', position: 'sticky', top: 0, zIndex: 100, background: 'rgba(26,10,10,0.75)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ width: '40px', height: '40px', borderRadius: '11px', background: brand, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '15px', color: 'white', boxShadow: `0 4px 14px ${brandGlow}` }}>EP</div>
          <div>
            <div style={{ fontSize: '1.15rem', fontWeight: 800, color: 'white', letterSpacing: '-0.02em' }}>EcoPulse</div>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={() => navigate('/login')} style={{ background: 'transparent', color: 'rgba(255,255,255,0.6)', border: '1px solid rgba(255,255,255,0.1)', fontSize: '0.9rem', fontWeight: 600, cursor: 'pointer', padding: '9px 20px', borderRadius: '9px', transition: 'all 0.2s', fontFamily: 'inherit' }}
            onMouseOver={e => { e.currentTarget.style.borderColor = `${brand}66`; e.currentTarget.style.color = 'white'; }}
      ...
      <section style={{ padding: '96px 6% 72px', textAlign: 'center', position: 'relative', zIndex: 1 }}>
        <motion.div initial="hidden" animate="visible" variants={stagger} style={{ maxWidth: '800px', margin: '0 auto' }}>
          ...
          <motion.h1 variants={fadeIn} style={{ fontSize: '4rem', fontWeight: 900, lineHeight: 1.1, marginBottom: '24px', letterSpacing: '-0.04em', color: 'white' }}>
            Hardware Engineering,{' '}
            <span style={{ color: brand }}>Version Controlled.</span>
          </motion.h1>

          <motion.p variants={fadeIn} style={{ fontSize: '1.1rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.75, maxWidth: '560px', margin: '0 auto 48px' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55, duration: 0.45 }}
          style={{ display: 'flex', justifyContent: 'center', marginTop: '64px', maxWidth: '480px', margin: '64px auto 0', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', overflow: 'hidden' }}>
          {STATS.map((s, i) => (
            <div key={i} style={{ flex: 1, padding: '22px 12px', textAlign: 'center', borderRight: i < STATS.length - 1 ? '1px solid rgba(255,255,255,0.07)' : 'none' }}>
              <div style={{ fontSize: '1.75rem', fontWeight: 900, color: brand }}>{s.value}</div>
              <div style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.4)', fontWeight: 500, marginTop: '4px' }}>{s.label}</div>
            </div>
          ))}
        </motion.div>
      </section>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-60px' }} variants={stagger}
          ...
          {FEATURES.map((f, i) => (
            <motion.div key={i} variants={fadeIn}
              style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '20px', padding: '34px', cursor: 'default', transition: 'all 0.3s' }}
              onMouseOver={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.borderColor = `${brand}40`; e.currentTarget.style.transform = 'translateY(-4px)'; }}
              ...
              <h3 style={{ fontSize: '1.15rem', fontWeight: 700, marginBottom: '10px', color: 'white', letterSpacing: '-0.02em' }}>{f.title}</h3>
              <p style={{ color: 'rgba(255,255,255,0.45)', lineHeight: 1.7, fontSize: '0.9rem', margin: 0 }}>{f.desc}</p>
            </motion.div>
          ))}
        </motion.div>
      </section>
      ...
      <section style={{ padding: '0 6% 90px', position: 'relative', zIndex: 1 }}>
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }}
          style={{ maxWidth: '860px', margin: '0 auto', background: `${brand}12`, border: `1px solid ${brand}30`, borderRadius: '24px', padding: '56px 48px', textAlign: 'center' }}>
          <h2 style={{ fontSize: '2.2rem', fontWeight: 900, color: 'white', letterSpacing: '-0.04em', marginBottom: '14px' }}>
            Ready to take control of your<br />
            <span style={{ color: brand }}>engineering changes?</span>
          </h2>
          <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '1rem', marginBottom: '32px' }}>Join your team on EcoPulse — the PLM platform built for precision.</p>
      ...
      {/* ─── Footer ─── */}
      <footer style={{ padding: '24px 6%', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: brand, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '11px', color: 'white' }}>EP</div>
          <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.85rem' }}>EcoPulse PLM</span>
        </div>
        <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: '0.8rem', margin: 0 }}>© {new Date().getFullYear()} EcoPulse PLM. Engineered for hardware teams.</p>
        <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
          <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#22c55e', boxShadow: '0 0 5px #22c55e' }} />
          <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.75rem', fontWeight: 600 }}>All systems operational</span>
        </div>
      </footer>
    </div>
  );
